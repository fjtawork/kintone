...
import { useAppId, useRecordId } from '@/lib/useRouteParams';
...
import { RecordAclEditor, RecordAcl } from '@/features/records/components/RecordAclEditor';
import { format } from 'date-fns';
import { toast } from 'sonner';
import { FieldType } from '@/features/app-builder/types';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export default function RecordPageClient() {
    ...
    const handleSave = (data: Record<string, unknown>) => {
        updateRecord({ data, acl: acl ?? null }, {
            onSuccess: () => {
    ...
    const renderFieldValue = (field: FieldDetails, value: unknown) => {
        ...
        switch (field.type) {
            case 'DATE':
                try { return format(new Date(value as string), 'yyyy-MM-dd'); } catch { return String(value); }
            case 'DATETIME':
                try { return format(new Date(value as string), 'yyyy-MM-dd p'); } catch { return String(value); }
            case 'CHECKBOX':
